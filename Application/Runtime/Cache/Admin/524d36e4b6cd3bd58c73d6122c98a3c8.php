		...
		<div id="tab-system" class="HuiTab">
			...
			<div class="tabCon">
				<div class="all-carousel-igure">
					<?php if(is_array($carousel_igure)): $kI = 0; $__LIST__ = $carousel_igure;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voData): $mod = ($kI % 2 );++$kI;?><div class="one-carousel-igure" id="one-carousel-igure-<?php echo ($kI); ?>" jayValue="<?php echo ($kI); ?>" >
						     <input type="hidden" name="carouselFigure[<?php echo ($kI); ?>][id]" value="<?php echo ($voData["id"]); ?>">
							<div class="row cl">
								<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>图片：</label>
								<div class="formControls col-xs-8 col-sm-4">
									<input  type="text" placeholder="请选择图片"  value="" class="input-text picture-text">
									<input type="hidden" value="<?php echo ($voData["picture_address"]); ?>" name="carouselFigure[<?php echo ($kI); ?>][picture_address]" class="picture-addr-box">
								</div>
								<div class="formControls col-xs-8 col-sm-2">
									<img src="/Albumsys/<?php echo ($voData["picture_address"]); ?>" style="width: 250px">
								</div>
								<div class="formControls col-xs-8 col-sm-2 " >
							...
							<div class="row cl">
								...
								<div class="formControls col-xs-8 col-sm-4">
									<input type="text" name="carouselFigure[<?php echo ($kI); ?>][title]"  placeholder="商品名称" value="<?php echo ($voData["title"]); ?>" class="input-text">
								</div>
							</div>
							<div class="row cl" >
								<label class="form-label col-xs-4 col-sm-2">说明：</label>
								<div class="formControls col-xs-8 col-sm-4">
									<textarea name="carouselFigure[<?php echo ($kI); ?>][introduction]" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="textarealength(this,1000)"><?php echo ($voData["introduction"]); ?></textarea>
